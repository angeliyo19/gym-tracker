...
def _get_plan_or_404(db: Session, plan_id: int) -> PlanAlimentacion:
    plan = (
        db.query(PlanAlimentacion)
        .options(_CARGA_DIAS)
        .filter(PlanAlimentacion.id == plan_id)
        .first()
    )
    if plan is None:
        raise HTTPException(
...
def crear_plan(plan_in: PlanAlimentacionCreate, db: Session = Depends(get_db)) -> PlanAlimentacion:
    _validar_usuario(db, plan_in.usuario_id)
    _validar_comidas(db, plan_in.dias)

    plan = PlanAlimentacion(usuario_id=plan_in.usuario_id, nombre=plan_in.nombre)
    db.add(plan)
    db.flush()
...
def listar_planes(
    ...
    return db.query(PlanAlimentacion).options(_CARGA_DIAS).offset(skip).limit(limit).all()


@router.get("/{plan_id}", response_model=PlanAlimentacionRead)
def obtener_plan(plan_id: int, db: Session = Depends(get_db)) -> PlanAlimentacion:
    return _get_plan_or_404(db, plan_id)


@router.patch("/{plan_id}", response_model=PlanAlimentacionRead)
def actualizar_plan(
    plan_id: int, plan_in: PlanAlimentacionUpdate, db: Session = Depends(get_db)
) -> PlanAlimentacion:
...
def eliminar_plan(plan_id: int, db: Session = Depends(get_db)) -> None:
    plan = _get_plan_or_404(db, plan_id)
    try:
